...
import { SparklesIcon, FacebookIcon, ExclamationTriangleIcon, SunIcon, MoonIcon, PhotoIcon } from './components/icons';
...
const App: React.FC = () => {
  ...
  const [isFbConfigMissing, setIsFbConfigMissing] = useState(false);
  ...
  // Load scheduled posts from localStorage on initial render
  useEffect(() => {
  ...
  useEffect(() => {
    if (FACEBOOK_APP_ID === 'YOUR_FACEBOOK_APP_ID' || !FACEBOOK_APP_ID) {
      setIsFbConfigMissing(true);
      return;
    }
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    setImageSource(null);
    try {
      const content = await generatePostContent(topic, tone, cta);
  ...
  const handleGenerateImage = async () => {
    ...
    setError(null);
    try {
      const imageUrl = await generatePostImage(topic || generatedPost);
      ...
      scheduledPosts.forEach(async post => {
        if (post.status === PostStatus.SCHEDULED && new Date(post.scheduledAt) <= now) {
          ...
          if(result.id || result.post_id) {
            ...
            // Handle failed auto-publish, maybe set status to 'failed' in future
      ...
      <header className="bg-white/80 dark:bg-slate-900/70 dark:border-b dark:border-slate-800 shadow-sm backdrop-blur-md sticky top-0 z-10">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
                ...
                <a href="https://ai.google.dev" target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-500">Powered by Gemini</a>
            </div>
        </div>
      </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white dark:bg-slate-900 p-6 rounded-lg shadow-lg flex flex-col gap-8">
            ...
            <div id="facebook-section" className="border-t border-slate-200 dark:border-slate-800 pt-6">
                ...
                {isFbConfigMissing ? (
                  <div className="p-4 border-l-4 border-amber-400 bg-amber-50 dark:bg-amber-900/20"><p><strong>Acción requerida:</strong> Configura tu ID de Aplicación de Facebook en <code>App.tsx</code>.</p></div>
                ) : (
                  <>
                    {!isFbSdkLoaded && <div className="text-center">Cargando conexión con Facebook...</div>}
                    {isFbSdkLoaded && !isLoggedIn && (<button onClick={handleLogin} className="w-full flex justify-center items-center gap-3 bg-blue-600 text-white font-bold py-3 px-4 rounded-md"><FacebookIcon />Conectar</button>)}
                    {isLoggedIn && (
                                ...
                                <div>
                                    ...
                                    <div className="mt-6 border-t border-slate-200 dark:border-slate-800 pt-4">
                                        <div className="relative flex items-start">
                                            <input id="scheduling-checkbox" type="checkbox" checked={isScheduling} onChange={(e) => setIsScheduling(e.target.checked)} className="h-4 w-4 mt-1 rounded border-gray-300 text-indigo-600" disabled={!!editingPostId} />
                                            <label htmlFor="scheduling-checkbox" className="font-medium text-gray-900 dark:text-slate-100 ml-3">Programar para más tarde</label>
                                        </div>
                                        {isScheduling && (
          ...
          <div>
            ...
            <PostPreview 
              ...
              onPublish={editingPostId ? handleScheduleOrUpdate : (isScheduling ? handleScheduleOrUpdate : () => handlePublishNow('immediate'))}
